  ...
  return {
    ...
    resolveId: {
      ...
      handler(id) {
        ...
        if (picks.length) {
          ...
          originIdsWithVirtual.set(
            normalizePath(path.resolve(resolvedId)),
            resolvedPath
          );
    ...
    configureServer(server) {
      ...
      function reloadRoutes(file: string) {
        ...
        const modInfo = server.moduleGraph.getModuleById(
          RESOLVED_ROUTE_INFO_ID
        );
        if (modInfo) {
