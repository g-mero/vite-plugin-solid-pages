  ...
  const filteredBody = ast.body.filter((node) => {
    switch (node.type) {
      ...
      case 'ExportNamedDeclaration':
        node.specifiers = node.specifiers.filter((spec) =>
          spec.type === 'ExportSpecifier'
            ? picks.includes(
                spec.exported?.value ? spec.exported.value : spec.orig.value
              )
            : false
        );
      ...
      case 'ImportDeclaration': {
        ...
        const extension = node.source.value.split('.').pop();
        if (isStyleExt(extension)) {
          return hasCss;
        }
